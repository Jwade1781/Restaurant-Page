...
function DictionarytoDom(elements) {
    ...
    for (const [key] of Object.entries(elements)) {
        ...
        if (element["description"] !== undefined) newDom.textContent = element["description"];
        if (element["src"] !== undefined) newDom.src = element["src"]
        if (element["id"] !== undefined) newDom.id = element["id"];
